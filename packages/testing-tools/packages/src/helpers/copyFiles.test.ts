...
const listDirsDiffUsingGlobby = async (source: string, target: string) => {
...
const listDirsDiffUsingRSync = async (source: string, target: string) => {
...
const listDirsDifference = async (source: string, target: string) => {
  if (process.platform === 'win32') {
    return listDirsDiffUsingGlobby(source, target);
  } else {
    return listDirsDiffUsingRSync(source, target);
  }
};
...
const createTestContent = async (location: string) => {
  ...
  await writeFile(join(location, 'src', 'utils', 'helper.ts'), randomText(256));
...
it('works almost like rsync but uses globby to find files', async () => {
  const temporarySource = join(tmpdir(), 'copy-files-test', 'source-YmVuyA6l');
