 ...
 */
export function isNamespaceStatement(
	node: ts.Node
): node is ts.ModuleDeclaration {
	return (
		ts.isModuleDeclaration(node) && Boolean(node.flags & ts.NodeFlags.Namespace)
	);
}
...
export function getExportsForSourceFile(
	...
	const result: SourceFileExport[] = typeChecker
		...
		.map((symbol: ts.Symbol) => ({
			symbol,
			exportedName: symbol.escapedName as string,
	...
	result.forEach((exp: SourceFileExport) => {
		...
		exp.originalName =
			resolvedIdentifier !== undefined
				? resolvedIdentifier.getText()
				: (exp.symbol.escapedName as string);
	});
...
function resolveIdentifierBySymbol(
...
type NamedTupleMemberCompat = unknown extends NamedTupleMember
	? ts.Node & { name: ts.Identifier }
	: NamedTupleMember;

export function isNamedTupleMember(
	node: ts.Node
): node is NamedTupleMemberCompat {
	...
	type CompatibleTypeScript = CommonKeys extends never
		? typeof ts & CompatibilityTypeScriptPart
		: typeof ts;
