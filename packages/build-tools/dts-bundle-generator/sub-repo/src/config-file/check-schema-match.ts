...
export type SchemeDescriptor<T> = {
	[P in keyof T]-?: T[P] extends unknown[]
		? [SchemeDescriptor<T[P][number]>]
		: SchemeDescriptor<T[P]>;
};
...
export function checkSchemaMatch<T>(
	value: unknown,
...
function checkSchemaMatchRecursively<T>(
	...
	if (typeof schema === 'boolean' || typeof schema === 'string') {
		...
		if (value === undefined && schemaRequiredValues.has(schema)) {
			errors.push(
				`Value for "${prefix}" is required and must have type "${schemeType}"`
			);
			return false;
		}
		...
		if (value !== undefined && typeof schema !== valueType) {
			errors.push(
				`Type of values for "${prefix}" is not the same, expected=${schemeType}, actual=${valueType}`
			);
			return false;
		}
	...
	for (const valueKey of Object.keys(value as object)) {
		if (schema[valueKey as keyof T] === undefined) {
			errors.push(
				`Exceeded property "${valueKey}" found in ${
					prefix.length === 0 ? 'the root' : prefix
				}`
			);
			result = false;
		}
