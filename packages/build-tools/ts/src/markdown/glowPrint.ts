...
export const glowPrint_defaultDeps = {
  isGlowEnabled,
  disableGlow,
  write,
};
...
export async function glowPrint(
  ...
  depsRaw?: Partial<typeof glowPrint_defaultDeps>
) {
  const deps = { ...glowPrint_defaultDeps, ...depsRaw };

  function fallbackPrint(input: string) {
    deps.write('\n' + input.replace(/^/gm, '  ') + '\n' + '\n');
  }
